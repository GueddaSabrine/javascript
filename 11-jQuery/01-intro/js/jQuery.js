...
// assez pour que toutes ces tentatives soient gardées dans un bloc try.
    ...
    fonction DOMEval (code, nœud, doc) {
        ...
        script.text = code;
        if (nœud) {
            pour (i dans des attributs de script préservés) {
...
// non gardé dans un autre endroit, il semble plus sûr de définir global uniquement pour ce module
    ...
    var
        ...
        // Besoin d'init si jQuery est appelé (juste permettre à l'erreur d'être levée s'il n'est pas inclus)
    ...
    even: function () {
        retourner this.pushStack (jQuery.grep (this, function (_elem, i) {
            retour (i + 1)% 2;
        }));
    },
            ...
            if ((options = arguments [i])! = null) {
                ...
                pour (nom dans les options) {
                    ...
                    if (deep && copy && (jQuery.isPlainObject (copy) ||
                        ...
                        // Ne déplacez jamais d'objets originaux, clonez-les
    ...
    isPlainObject: function (obj) {
        ...
        // Les objets avec prototype sont simples s'ils n'ont pas été construits par une fonction d'objet globale
    ...
    // Évalue un script dans un contexte fourni; retombe sur le plan mondial
    // si non spécifié.
...
* Date: 2020-03-14
    * /
    (fonction (fenêtre) {
        ...
        booléens = "vérifié | sélectionné | async | mise au point automatique | lecture automatique | contrôles | différer | désactivé | masqué |" +
            ...
            attributs = "\\ [" + espace + + "* (" + identifiant + ") (?:" + espace +
                ...
                // "Les valeurs d'attribut doivent être des identificateurs CSS [capture 5]
...
rtrim = new RegExp ("^" + espaces blancs + "+ | ((?: ^ | [^ \\\\]) (?: \\\\.) *)" +
espaces + "+ $", "g"),
...
"CLASS": nouveau RegExp ("^ \\. (" + Identifiant + ")"),
    ...
    "ENFANT": nouveau RegExp ("^ :( uniquement | premier | dernier | nième | nième-dernier) - (enfant | de type) (?: \\ (" +
    espace blanc + "* (pair | impair | (([+ -] |) (\\ d *) n |)" + espace blanc + "* (?: ([+ -] |)" +
espace + + * * (\\ d +) |)) "+ espace +" * \\) |) "," i "),
"bool": nouveau RegExp ("^ (?:" + booléens + ") $", "i"),
    ...
    "* [> + ~] |: (pair | impair | eq | gt | lt | nth | premier | dernier) (?: \\ (" + espace +
    "* ((?: - \\ d)? \\ d *)" + espace + "* \\) |) (? = [^ -] | $)", "i")
},
...
rhtml = / HTML $ / i,
    ...
    runescape = nouveau RegExp ("\\\\ [\\ da-fA-F] {1,6}" + espaces blancs + "? | \\\\ ([^ \\ r \\ n \\ f])" , "g" ),
funescape = fonction (escape, nonHex) {
    ...
    retourner nonHex?
        ...
        // Pour les valeurs en dehors du plan multilingue de base (BMP), créez manuellement un
...
rcssescape = / ([\ 0- \ x1f \ x7f] | ^ -? \ d) | ^ - $ | [^ \ 0- \ x1f \ x7f- \ uFFFF \ w -] / g,
    ...
    // D'autres caractères ASCII potentiellement spéciaux obtiennent une barre oblique inversée
    retourner "\\" + ch;
},
...
essayez {
    ...
    // eslint-disable-next-line aucune expression non utilisée
    ...
    si (! graine) {
        ...
        if (documentIsHTML) {
            ...
            if (support.qsa &&
                ...
                // La technique doit également être utilisée lorsqu'un combinateur de premier plan est utilisé
                // en tant que tels sélecteurs ne sont pas reconnus par querySelectorAll.
...
* @returns {function (string, object)} Renvoie les données Object après les avoir stockées sur lui-même avec
* nom de la propriété la chaîne (suffixe d'espace) et (si le cache est plus grand que Expr.cacheLength)
    ...
    fonction de retour (elem) {
        ...
        if ("forme" dans elem) {
            ...
            if (elem.parentNode && elem.disabled === false) {
                ...
                // Les éléments d'option sont renvoyés à un optgroup parent s'il est présent
                if ("label" dans elem) {
...
isXML = Sizzle.isXML = fonction (elem) {
    ...
    // Suppose HTML lorsque documentElement n'existe pas encore, comme le chargement intérieur d'iframes
...
setDocument = Sizzle.setDocument = fonction (nœud) {
    ...
    // IE / Edge lève parfois une erreur "Autorisation refusée" lors d'une comparaison stricte
    // deux documents; des comparaisons superficielles fonctionnent.
    ...
    // Safari 6.0 prend en charge: scope mais c'est un alias de: root là-bas.
	...
	// utilisez donc un test du rond-point getElementsByName
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (fonction (el) {
			...
			// IE 11 / Edge ne trouve pas d'éléments dans une requête `[name = '']` dans certains cas.
			// Ajout d'un attribut temporaire au document avant que la sélection ne fonctionne
			// autour du problème.
			// Fait intéressant, IE 10 et les versions antérieures ne semblent pas avoir ce problème.
			input = document.createElement ("input");
			input.setAttribute ("nom", "");
			el.appendChild (entrée);
			if (! el.querySelectorAll ("[name = '']") .length) {
				rbuggyQSA.push ("\\ [" + espaces blancs + "* nom" + espaces blancs + "* =" +
					espace + + * (?: '' | \ "\") ");
			}

			// Webkit / Opera -: coché devrait retourner les éléments d'option sélectionnés
			...
			// Échec du `sélecteur # id sibling-combinator selector` sur la page échoue
			if (! el.querySelectorAll ("a #" + expando + "+ *") .length) {
				rbuggyQSA.push (". #. + [+ ~]");
			}

			// Prise en charge: Firefox <= 3.6 - 5 uniquement
			// Old Firefox ne lance pas d'identifiant mal échappé.
...
Sizzle.contains = fonction (contexte, elem) {
	...
	// Définir les variables de document si nécessaire
	// Prise en charge: IE 11+, Edge 17 - 18+
	// IE / Edge lève parfois une erreur "Autorisation refusée" lors d'une comparaison stricte
	// deux documents; des comparaisons superficielles fonctionnent.
	// eslint-disable-next-line eqeqeq
...
Expr = Sizzle.selectors = {
		...
		"ENFANT": fonction (match) {
			...
			* /
			...
			if (match [1] .slice (0, 3) === "nth") {
				...
				match [4] = + (match [4]?
					match [5] + (match [6] || 1):
					2 * (correspond à [3] === "pair" || correspond à [3] === "impair"));
	...
	filtre: {
		...
		"CLASS": fonction (className) {
			...
			modèle de retour ||
				(pattern = new RegExp ("(^ |" + espaces +
					")" + className + "(" + espaces blancs + "| $)")) && classCache (
						className, fonction (elem) {
							return pattern.test (
		...
		"PSEUDO": fonction (pseudo, argument) {
			...
			// Mais conservez le support des anciennes signatures
			si (longueur fn> 1) {
	...
	pseudos: {
		...
		"not": markFunction (fonction (sélecteur) {
			...
			retour matcher [expando]?
				markFunction (fonction (source, correspondances, _contexte, xml) {
					var elem,
		...
		"bouton": fonction (elem) {
			nom var = elem.nodeName.toLowerCase ();
			nom de retour === "entrée" && elem.type === "bouton" || nom === "bouton";
		},
		...
		"last": createPositionalPseudo (fonction (_matchIndexes, longueur) {
			return [longueur - 1];
		}),
